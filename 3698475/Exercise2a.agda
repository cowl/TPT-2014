...
data _⇓_ {ty : Type} : (Term ty) → (Value ty) → Set where
  terminates : {t : Term ty} -> {v : Value ty} -> (Steps t ⌜ v ⌝) -> t ⇓ v

-- Converstion from big- to small-step representations.
big-to-small : ∀ {ty} {t : Term ty} {v : Value ty} → t ⇓ v → Steps t ⌜ v ⌝
big-to-small (terminates x) = x

-- Conversion from small- to big-step representations.
value-to-value : forall {ty} (v : Value ty) -> ⌜ v ⌝ ⇓ v
value-to-value _ = terminates Nil

prepend-step : {ty : Type} -> (t t' : Term ty) (v : Value ty) → Step t t' -> t' ⇓ v → t ⇓ v
prepend-step t1 t2 v step (terminates  x) = terminates (Cons step x)

small-to-big : {ty : Type} -> (t : Term ty) -> (v : Value ty) → Steps t ⌜ v ⌝ → t ⇓ v
small-to-big _ _ steps = terminates steps


--dingetje : (t : Term NAT)(t' : Value NAT) -> t ⇓ t' -> (succ t) ⇓ (vsucc t')
--dingetje .(⌜ v ⌝) v (terminates Nil) = {!terminates E-Succ-Steps Nil!}
--dingetje t v (terminates (Cons x x₁)) = {!!}

--dingetje : ∀ {t : Term NAT} {t' : Value NAT} -> 
--        Steps t ⌜ t' ⌝ →
--        Steps (succ t) (⌜ vsucc t' ⌝)
--dingetje Nil = {!!}
--dingetje (Cons x steps) = Cons (E-Succ x) (dingetje steps)
--------------------------------------------------------------------------------
...
⇓-complete zero = terminates Nil
⇓-complete true = terminates Nil
⇓-complete false = terminates Nil
⇓-complete t with  ⟦ t ⟧
⇓-complete true | vtrue = terminates Nil
⇓-complete false | vtrue = {!!}
⇓-complete (if t then t₁ else t₂) | vtrue = {!!}
⇓-complete (iszero t) | vtrue = {!!}
⇓-complete t | vfalse = {!!}
⇓-complete t | vnat x = {!!} 
...
-- ⇓-sound t v (terminates x) with  ⟦ t ⟧
-- ⇓-sound t vtrue (terminates x) | vtrue = refl
-- ⇓-sound t vfalse (terminates x) | vtrue = {!!} -- wut.
-- ⇓-sound t vtrue (terminates x) | vfalse = {!!}
-- ⇓-sound t vfalse (terminates x) | vfalse = refl
-- ⇓-sound t (vnat Zero) (terminates x₁) | vnat Zero = refl
-- ⇓-sound t (vnat (Succ x)) (terminates x₁) | vnat Zero = {!!} -- =|
-- ⇓-sound t (vnat Zero) (terminates x₂) | vnat (Succ x2) = {!!}
-- ⇓-sound t (vnat (Succ x1)) (terminates x₂) | vnat (Succ x2) = {!!}


⇓-sound : ∀ {ty} (t : Term ty) (v : Value ty) → t ⇓ v → v == ⟦ t ⟧
⇓-sound t v p with ⌜ v ⌝
⇓-sound true vfalse (terminates (Cons () x₁)) | _
⇓-sound false vtrue (terminates (Cons () x₁)) | _
⇓-sound false vfalse p | _ = refl
⇓-sound true vtrue p | _ = refl

⇓-sound (if t then t1 else t2) v p | true = {!!}




⇓-sound (iszero t) v p | true = {!!}

⇓-sound (if t then t₁ else t₂) vtrue p | false = {!!}
⇓-sound (iszero t) vtrue p | false = {!!}
⇓-sound (if t then t₁ else t₂) vfalse p | false = {!!}
⇓-sound (iszero t) vfalse p | false = {!!}
⇓-sound t v p | if q then q₁ else q₂ = {!!}
⇓-sound t v p | zero = {!!}
⇓-sound t v p | succ q = {!!}
⇓-sound t v p | iszero q = {!!} 
